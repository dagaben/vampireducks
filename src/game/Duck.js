...
/**
 * Rubber Vampire Duck — follows terrain height.
 */
export class Duck {
  ...
  update(delta, playerPos, isNight, world) {
    ...
    // Follow terrain + small waddle bob
    const terrainY = world ? world.getHeightAt(this.group.position.x, this.group.position.z) : 0;
    this.group.position.y = terrainY + 0.55 + Math.sin(Date.now() * 0.012) * 0.08;
  }
...
export class DuckManager {
  ...
  update(delta, player, isNight, world, onHit) {
    for (let i = this.ducks.length - 1; i >= 0; i--) {
      const d = this.ducks[i];
      d.update(delta, player.position, isNight, world);
      ...
      if (d.state === 'alive') {
        ...
        if (dist < d.radius + player.radius) {
          if (player.position.y > player.position.y * 0 + 1.4 && player.velocity.y < 0) {
            player.bounce();
          } else {
            onHit(d);
          }
